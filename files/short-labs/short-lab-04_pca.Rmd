...
**3**. Plot your first two principal components (see slides 35 and 36 of Slides 5). Label between $5$ and $10$ countries. Interpret your results. Which countries have similar environmental indicators? Which countries have different environmental indicators? 

**4**. Take you plot from (**3**) and color the points by one of the variables in `other_df`. Choose either `region`, `ldc` (which is $1$ for a set of least developed countries and $0$ otherwise), `emmrk` (which is $1$ if the country has an emerging market and $0$ otherwise), or `gdp` (if you choose `gpd` I recommend instead using `log(gdp)` because it is a very right-skewed variable). See slides 41 through 46 of Slides 5 for code inspiration. 
