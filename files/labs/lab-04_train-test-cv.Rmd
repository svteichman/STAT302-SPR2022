...
To begin, load in the popular `penguins` data set from the package `palmerpenguins`. Use the dataframe `peng_clean` for your analysis to avoid missing data. 
...
**Submission:** To prove your function works, apply it to the `penguins` data (using `peng_clean`). Predict output class `species` using covariates `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm`, and `body_mass_g`. Use $5$-fold cross validation (`k_cv = 5`). Use a table to show the `cv_err` values for 1-nearest neighbor and 5-nearest neighbors (`k_nn = 1` and `k_nn = 5`), as well as the training set error values for 1 and 5 nearest neighbors (compare your output `class` to the true class, `penguins$species`). Comment on which value had lower CV misclassification error and which had lower training set error. Which model would you choose based on this analysis? Why?