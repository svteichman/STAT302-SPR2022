...
Let's jump right in and start working with large simulations. You will need the functions `rnorm()` for Normally distributed simulations, `pnorm()` for the percentiles of the Normal distribution, and `dnorm()` for the density of the Normal distribution. This is all I am going to tell you about these functions, so you will need to use the documentation to help you with these questions! In order to open the documentation (for `rnorm()` for example) go into the console and type `?rnorm()`. 
...
**1d.** In order to standardize vectors, we take each element and subtract the mean and then divide by the standard deviation (computed in part c). Create and store a new vector that is the standardization of your simulations from part (a). Create a histogram for these standardized simulations (don't forget to change the title again!). What do you notice?
...
**1g.** How does the quantity from part d compare to the observed proportion of your first 10 standardized simulations that are above 1.644854? Repeat this for your first 100, 1000, and 10,000 standardized simulations. 
What do you notice?
